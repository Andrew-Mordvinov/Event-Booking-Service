...
| Метод   | URL          | Описание                                                           |
|---------|--------------|--------------------------------------------------------------------|
| GET     | /event       | Возвращает список всех мероприятий                                 |
| GET     | /event/{id}  | Возвращает мероприятие с указанным ID                              |
| POST    | /event       | Создает новое мероприятие. При успехе возвращает 201 Created       |
| PUT     | /event/{id}  | Полностью обновляет мероприятие с указанным ID                     |
| DELETE  | /event/{id}  | Удаляет мероприятие с указанным ID                                 |

Документация API доступна по адресу `/swagger` при запуске проекта в режиме Develop